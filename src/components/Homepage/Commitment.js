// src/components/About.js
import React from "react";


export default function Commitment({commitment}) {
  if (!commitment) return null; // Don't render if nothing is visible

  return (
    <section id="about" className="py-5 bg-white text-center">
      <div className="container" data-aos= 'fade-right' data-aos-duration="2000">
        <h2 className="text-success fw-bold mb-2">{commitment.title}</h2>
        <div className="section-underline" style={{ width: '200px'}}></div>
        <p className="lead">
        {commitment.content}
        </p>
      </div>
    </section>
  );
}


// return (
//   <section id="about" className="py-5 bg-white text-center">
//     <div className="container" data-aos= 'fade-right' data-aos-duration="2000">
//       <h2 className="text-success fw-bold mb-2">What do we do</h2>
//       <div className="section-underline" style={{ width: '200px'}}></div>
//       <p className="lead">
//       we are commited onfocuses on promoting inclusive development through education, digital literacy, civic engagement, and access to welfare schemes.

//       <br/>
//       <br/>
//       PCWT aims to bridge the gap between communities and essential services by fostering awareness and 
//       participation at the local level. By working closely with residents, we strive to build a more informed,
//        self-reliant, and connected society in Pulakeshi Nagar and beyond.
//       </p>
//     </div>
//   </section>
// );
// }
